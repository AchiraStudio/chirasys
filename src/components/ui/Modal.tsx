...
export default function Modal({
  ...
  const modalContent = (
    <div
      className={`fixed inset-0 z-[1000] flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-x-hidden ${backdropClassName}`}
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      {/* Modal Dialog Card */}
      <div
        className={`relative z-10 w-full ${SIZE_CLASSES[size]} bg-elevated text-heading rounded-xl border border-line shadow-2xl shadow-black/40 flex flex-col max-h-[95vh] sm:max-h-[92vh] overflow-hidden animate-pop-in ${className}`}
      ...
      >
        ...
        {footer && (
          <div
            className={`px-4 sm:px-6 py-3 sm:py-4 border-t border-line flex items-center justify-end gap-2 sm:gap-3 shrink-0 ${footerClassName}`}
          >
            {footer}
          </div>
        )}
