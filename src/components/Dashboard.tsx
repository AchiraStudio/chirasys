  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold tracking-tight">Overview</h1><p className="text-sm text-slate-500 mt-1">Here is what's happening today.</p></div>
        <button className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 rounded-lg text-sm font-medium">Export Report</button>
      </div>
            ...
            {stats.map((stat, i) => {
              ...
              return (
                <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 shadow-sm relative group">
                  <div className="flex justify-between items-start mb-4"><div className={`p-3 rounded-xl ${stat.bg} ${stat.color}`}><Icon size={22} strokeWidth={2.5} /></div><button className="text-slate-400"><MoreHorizontal size={20} /></button></div>
                  <div><h3 className="text-3xl font-bold">{stat.value}</h3><div className="flex items-center justify-between mt-2"><p className="text-sm font-medium text-slate-500">{stat.title}</p><div className="flex items-center text-xs font-semibold">{stat.isUp ? <ArrowUpRight size={14} className="text-emerald-500 mr-1" /> : <ArrowDownRight size={14} className="text-rose-500 mr-1" />}<span className={stat.isUp ? 'text-emerald-600' : 'text-rose-600'}>{stat.trend}</span></div></div></div>
                </div>
              );
            })}
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4">
              {alerts.length === 0 ? <p className="text-sm text-slate-500 text-center py-4">All stock levels look good!</p> : alerts.map((alert, i) => (
                <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="flex gap-3 items-center">
                    ...
                    <div><p className="font-semibold text-sm">{alert.item_name}</p><p className="text-xs text-slate-500 mt-0.5">SKU: {alert.sku}</p></div>
                  </div>
                  <div className="text-right"><p className="text-sm font-bold text-rose-600">{alert.current_qty} left</p><p className="text-[10px] text-slate-400 uppercase tracking-wider mt-1">Min: {alert.min_stock}</p></div>
                </div>
              ))}
            </div>
