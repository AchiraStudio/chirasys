...
import { Save, Plus, Trash2, Percent, Coins, Gift, Layers, Package, Calendar, Settings, Info, Search, Sparkles, Tag } from 'lucide-react';
...
export default function PromoDrawer({ isOpen, onClose, onSaved, editPromoId }: PromoDrawerProps) {
    ...
    try {
      ...
      if (promo.item_id) {
        try {
          ...
          setSelectedItemName(itemData.item.name);
  ...
  const handleAddTier = () => {
    setFormData(prev => ({
      ...prev,
      tiers: [...prev.tiers, { min_qty: 1, discount_percent: 0 }]
    }));
  };
  ...
  return (
    <Modal
      ...
      subtitle="Configure pricing rules, discounts, and promotional logic"
      icon={Sparkles}
      footer={
        <div className="flex items-center justify-end gap-3 w-full">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 rounded-2xl font-bold text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            form="promo-form"
            disabled={loading}
            className="px-6 py-2.5 bg-brand hover:bg-brand/90 active:scale-[0.98] text-white rounded-2xl font-bold text-sm shadow-lg shadow-brand/20 transition-all flex items-center gap-2 disabled:opacity-50"
          >
            <Save size={16} />
            {loading ? 'Saving...' : editPromoId ? 'Update Promotion' : 'Create Promotion'}
          </button>
        </div>
      }
    >
        ...
        <div className="space-y-4">
          <div>
            ...
            <input 
              ...
              className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand/35 focus:border-brand transition-all duration-200 font-semibold" 
              placeholder="e.g. Weekend Flash Sale 20%" 
            />
          </div>
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand/35 focus:border-brand transition-all duration-200 text-sm font-medium resize-none" 
              rows={2} 
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="fixed_amount">Fixed Amount Discount (Rp)</option>
          ...
          <div>
            ...
            <div className="relative">
              <select 
                ...
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand/35 focus:border-brand transition-all duration-200 font-semibold appearance-none cursor-pointer"
              >
                <option value="item">Specific Single Item</option>
                <option value="category">Whole Product Category</option>
                ...
                <div className="grid grid-cols-2 gap-4 items-end bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/40">
                  ...
                  <div className="col-span-2 mt-2">
                    <div className="flex items-center gap-2 p-3 bg-amber-500/10 text-amber-600 rounded-xl border border-amber-500/20 text-[11px] font-semibold">
                      ...
                      <span>Under current BOGO configuration, the free item will match the purchased item.</span>
                    </div>
                  </div>
                </div>
              )}
