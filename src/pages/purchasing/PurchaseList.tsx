  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in h-full">
      <div>
        ...
        <p className="text-sm text-slate-600 mt-1">Supplier invoices and received stock.</p>
      </div>
              ...
              {purchases.length === 0 ? (
                ...
                <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 group">
                  ...
                  <td className="py-4 px-6 text-right">
                    <button
                      onClick={() => onView(p.id)}
                      className="p-2 text-slate-500 hover:text-brand bg-slate-100 dark:bg-slate-800 rounded-md opacity-0 group-hover:opacity-100 transition-opacity"
                      title="View Details"
                    >
                      <Eye size={14} />
                    </button>
