    ...
    <div className="flex flex-col gap-6 animate-in fade-in h-full">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage POS clients and loyalty tiers.</p>
        </div>
              ...
              {customers.map(c => (
                <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 group">
                  ...
                  <td className="py-4 px-6 text-right">
                    <button onClick={() => openEdit(c)} className="p-2 text-slate-400 hover:text-brand bg-slate-100 dark:bg-slate-800 rounded-md opacity-0 group-hover:opacity-100 transition-opacity"><Edit2 size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
