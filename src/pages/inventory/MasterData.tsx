        ...
        {children.map(cat => (
          <div key={cat.id}>
            <div className="flex items-center gap-3 py-2 px-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg transition-colors group" style={{ marginLeft: `${depth * 1.5}rem` }}>
              {depth > 0 && <ChevronRight size={14} className="text-slate-300 dark:text-slate-600" />}
              <FolderTree size={16} className={depth === 0 ? "text-brand" : "text-slate-400"} />
              {editingId === cat.id ? (
                <div className="flex-1 flex items-center gap-2">
                  ...
                  <button onClick={() => setEditingId(null)} className="text-slate-400 hover:text-rose-500"><X size={16} /></button>
                </div>
              ) : (
                <>
                  ...
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                    <button onClick={() => { setEditingId(cat.id); setEditName(cat.name); }} className="text-slate-400 hover:text-brand p-1"><Edit2 size={14} /></button>
                    <button onClick={() => handleDelete(cat.id)} className="text-slate-400 hover:text-rose-500 p-1"><Trash2 size={14} /></button>
                  </div>
                </>
              )}
            </div>
    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage your product categories and brand registries.</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#0B0F19] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
        <div className="flex border-b border-slate-200 dark:border-slate-800 px-6 pt-4 bg-slate-50/50 dark:bg-slate-900/30">
          <button onClick={() => { setActiveTab('categories'); setEditingId(null); }} className={`flex items-center gap-2 px-4 py-3 border-b-2 transition-all font-medium text-sm ${activeTab === 'categories' ? 'border-brand text-brand' : 'border-transparent text-slate-500'}`}><FolderTree size={18} />Categories</button>
          <button onClick={() => { setActiveTab('brands'); setEditingId(null); }} className={`flex items-center gap-2 px-4 py-3 border-b-2 transition-all font-medium text-sm ${activeTab === 'brands' ? 'border-brand text-brand' : 'border-transparent text-slate-500'}`}><Tag size={18} />Brands</button>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col">
          ...
          <div className="flex-1">
            {loading ? <div className="flex flex-col items-center py-20 text-slate-400"><Loader2 size={32} className="animate-spin mb-4 text-brand" /><p>Loading registry data...</p></div> : 
             activeTab === 'categories' ? (
              <div className="bg-white dark:bg-[#0B0F19] rounded-xl border border-slate-200 dark:border-slate-800 p-4">{categories.length === 0 ? <p className="text-center text-slate-500 py-10">No categories found.</p> : renderCategoryTree(null, 0)}</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {brands.length === 0 ? <p className="text-center text-slate-500 py-10 col-span-full">No brands found.</p> : brands.map(b => (
                  <div key={b.id} className="group p-4 border border-slate-200 dark:border-slate-800 rounded-xl flex justify-between items-center bg-white dark:bg-[#0B0F19] hover:border-brand/30 transition-colors shadow-sm">
                    {editingId === b.id ? (
                      ...
                      <><div className="flex items-center gap-3"><div className="p-2 bg-slate-50 dark:bg-slate-900 rounded-lg text-slate-400"><Tag size={16} /></div><span className="font-semibold text-sm">{b.name}</span></div>
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                          <button onClick={() => { setEditingId(b.id); setEditName(b.name); }} className="p-1.5 text-slate-400 hover:text-brand hover:bg-slate-50 dark:hover:bg-slate-900 rounded-md"><Edit2 size={14} /></button>
                          <button onClick={() => handleDelete(b.id)} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-md"><Trash2 size={14} /></button>
                        </div>
                      </>
                    )}
                  </div>
