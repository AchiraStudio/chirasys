    ...
    <div className="flex flex-col gap-6 animate-in fade-in h-full">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage vendor records for Phase 4 purchasing.</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-sm">
              {suppliers.map(s => (
                <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 group">
                  <td className="py-4 px-6 font-bold flex items-center gap-3"><Building2 size={16} className="text-slate-400" />{s.name}</td>
                  <td className="py-4 px-6">{s.contact_person || '-'}</td>
                  <td className="py-4 px-6 font-mono text-slate-500">{s.phone || '-'}</td>
                  <td className="py-4 px-6 text-right">
                    <button onClick={() => openEdit(s)} className="p-2 text-slate-400 hover:text-brand bg-slate-100 dark:bg-slate-800 rounded-md opacity-0 group-hover:opacity-100 transition-opacity"><Edit2 size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
