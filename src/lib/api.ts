...
export const invoke = async <T>(cmd: string, args?: Record<string, any>): Promise<T> => {
  // 1. If running in Web Browser (Mobile Phone, Tablet, Remote Browser)
  if (!isTauri()) {
    const hostUrl = getHostUrl();
    try {
      const response = await fetch(`${hostUrl}/api/lan/rpc`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(`Host Error (${response.status}): ${errText}`);
      }

      const json = await response.json();
      if (!json.success) {
        throw new Error(json.error || 'Terjadi kesalahan pada Server Kivo Host');
      }

      return json.data as T;
    } catch (err: any) {
      // Fallback: If same-origin proxy failed, try direct port 3699
      if (typeof window !== 'undefined' && !hostUrl.includes(':3699')) {
        try {
          const directUrl = `http://${window.location.hostname || 'localhost'}:3699`;
          const response = await fetch(`${directUrl}/api/lan/rpc`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ command: cmd, params: args || {} }),
      ...
      console.warn(`[Web Client RPC] Failed to call ${cmd} on host (${hostUrl}):`, err);
      if (err.name === 'TimeoutError' || (err.message && (err.message.includes('Failed to fetch') || err.message.includes('network') || err.message.includes('timed out')))) {
        throw new Error(`Tidak dapat terhubung ke Kivo Host di ${hostUrl}. Pastikan perangkat terhubung ke Wi-Fi yang sama.`);
      }
      throw err;
    }
  }

  // 2. If running inside native Tauri Desktop
