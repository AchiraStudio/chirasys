...
pub fn notify_mutation(table: &str) {
    ...
    println!("⚡ [Realtime LAN] Mutation broadcasted: table='{}', version={}", table, new_v);

    // Asynchronously notify desktop Tauri window
...
pub async fn handle_lan_poll(
    ...
    // Wait for the next mutation or timeout
...
async fn handle_active_session(AxumState(ctx): AxumState<ServerContext>) -> Json<serde_json::Value> {
    ...
    // 2. Check local_sessions for latest active session
    ...
    // 3. Fallback: Find primary active owner/admin/manager user and auto-generate an active 24-hour session
    ...
    if let Some(uid) = fallback_user_id {
        ...
        let expires_at = (chrono::Utc::now() + chrono::Duration::try_hours(24).unwrap_or(chrono::Duration::hours(24))).to_rfc3339();
...
async fn handle_lan_rpc(
    ...
    let res: Result<serde_json::Value, String> = match cmd {
        ...
        "get_current_user" => {
            ...
            let res = crate::commands::auth::get_current_user(token.clone(), state.clone()).await;
            if let Ok(user) = res {
                Ok(serde_json::to_value(user).unwrap_or_default())
            } else {
                // Fallback to active host session so terminal client is never locked out
                let fallback_token: Option<String> = sqlx::query_scalar(
                    "SELECT value FROM global_settings WHERE key = 'active_host_token' AND value != ''"
                )
                .fetch_optional(&ctx.pool)
                .await
                ...
                if let Some(ft) = fallback_token {
                    if let Ok(u) = crate::commands::auth::get_current_user(ft, state.clone()).await {
                        resolved_user = Some(u);
                    }
                }

                if resolved_user.is_none() {
                    let latest_token: Option<String> = sqlx::query_scalar(
                        "SELECT token FROM local_sessions WHERE expires_at > datetime('now') ORDER BY created_at DESC LIMIT 1"
                    )
                    .fetch_optional(&ctx.pool)
                    .await
                    ...
                    if let Some(lt) = latest_token {
                        if let Ok(u) = crate::commands::auth::get_current_user(lt, state.clone()).await {
                            resolved_user = Some(u);
                        }
                    }
                }
                ...
                if let Some(u) = resolved_user {
                    Ok(serde_json::to_value(u).unwrap_or_default())
                } else {
                    Err("Sesi tidak valid atau telah kedaluwarsa.".to_string())
                }
            }
        }
        "get_users" => {
        ...
        "toggle_user_active" => {
            let id: String = p.get("id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            crate::commands::auth::toggle_user_active(id, state).await.map(|_| serde_json::Value::Null)
        }
        "reset_user_password" => {
            let id: String = p.get("id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let new_password: String = p.get("newPassword").or_else(|| p.get("new_password")).and_then(|v| v.as_str()).unwrap_or_default().to_string();
            crate::commands::auth::reset_user_password(id, new_password, state).await.map(|_| serde_json::Value::Null)
        }
        "delete_user" => {
            let id: String = p.get("id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            crate::commands::auth::delete_user(id, state).await.map(|_| serde_json::Value::Null)
        }
        "create_user" => {
            let name: String = p.get("name").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let username: String = p.get("username").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let password: String = p.get("password").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let role: String = p.get("role").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            ...
            crate::commands::auth::create_user(name, username, password, role, workspace_id, state).await.map(|d| serde_json::to_value(d).unwrap_or_default())
        }
        "update_user" => {
            let id: String = p.get("id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let name: String = p.get("name").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            ...
            let role: String = p.get("role").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let workspace_id: Option<String> = p.get("workspaceId").or_else(|| p.get("workspace_id")).and_then(|v| serde_json::from_value(v.clone()).ok()).flatten();
            crate::commands::auth::update_user(id, name, username, role, workspace_id, state).await.map(|_| serde_json::Value::Null)
        }
        "update_role_default_permissions" => {
            let role: String = p.get("role").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let permissions: Vec<String> = p.get("permissions").and_then(|v| serde_json::from_value(v.clone()).ok()).unwrap_or_default();
            crate::commands::auth::update_role_default_permissions(role, permissions, state).await.map(|_| serde_json::Value::Null)
        }
        "update_user_permissions" => {
            let user_id: String = p.get("userId").or_else(|| p.get("user_id")).and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let is_custom: bool = p.get("isCustom").or_else(|| p.get("is_custom")).and_then(|v| v.as_bool()).unwrap_or(false);
            let permissions: Vec<String> = p.get("permissions").and_then(|v| serde_json::from_value(v.clone()).ok()).unwrap_or_default();
            crate::commands::auth::update_user_permissions(user_id, is_custom, permissions, state).await.map(|_| serde_json::Value::Null)
        }
        "auto_assign_brands" => {
            crate::commands::masters::auto_assign_brands(state).await.map(|d| serde_json::to_value(d).unwrap_or_default())
        }
        "bulk_add_stock" => {
            let branch_id: String = p.get("branchId").or_else(|| p.get("branch_id")).and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let items: Vec<crate::commands::inventory::BulkStockInput> = p.get("items").and_then(|v| serde_json::from_value(v.clone()).ok()).unwrap_or_default();
            crate::commands::inventory::bulk_add_stock(branch_id, items, state).await.map(|d| serde_json::to_value(d).unwrap_or_default())
        }
        "send_ai_chat_request" => {
            let request: crate::commands::ai::ChatCompletionRequest = p.get("request")
                .and_then(|v| serde_json::from_value(v.clone()).ok())
                .unwrap_or(crate::commands::ai::ChatCompletionRequest {
                    model: None,
            ...
            crate::commands::ai::send_ai_chat_request(request, state).await
        }
        "get_sync_status" => {
            crate::commands::sync::get_sync_status(state).await.map(|d| serde_json::to_value(d).unwrap_or_default())
        }
        "get_lan_status" => {
            crate::commands::lan::get_lan_status(state).await.map(|d| serde_json::to_value(d).unwrap_or_default())
        }
        "get_lan_peers" => {
            crate::commands::lan::get_lan_peers(state).await.map(|d| serde_json::to_value(d).unwrap_or_default())
        }
        "kick_cash_drawer" => {
            let printer_name = p.get("printerName").or_else(|| p.get("printer_name")).and_then(|v| v.as_str()).unwrap_or_default().to_string();
        ...
        "logout" => Ok(serde_json::Value::Null),
        "receive_cloud_sync" => Ok(serde_json::Value::Null),

        unknown => Err(format!("Perintah RPC '{}' tidak didukung oleh Server Induk", unknown)),
    };

    // If a mutating RPC command succeeded, broadcast the change to all connected devices & host
    let is_mutation = match cmd {
        ...
        "get_all_permissions" | "get_role_default_permissions" | "get_user_permissions" | "get_sync_status" |
        "get_lan_status" | "get_lan_peers" | "list_printers" | "send_ai_chat_request" | "login" | "logout" => false,
        _ => true,
    };
