...
#[tauri::command]
pub async fn get_promos(active_only: bool, state: State<'_, AppState>) -> Result<Vec<Promo>, String> {
...
#[tauri::command]
pub async fn get_promo_detail(id: String, state: State<'_, AppState>) -> Result<PromoDetail, String> {
    let promo = sqlx::query_as::<_, Promo>("SELECT * FROM promos WHERE id = ?")
        .bind(&id).fetch_one(&state.db_pool).await.map_err(|e| e.to_string())?;

    let bogo_rules = sqlx::query_as::<_, PromoBogoRule>("SELECT * FROM promo_bogo_rules WHERE promo_id = ?")
        .bind(&id).fetch_all(&state.db_pool).await.map_err(|e| e.to_string())?;

    let tiers = sqlx::query_as::<_, PromoTier>("SELECT * FROM promo_tiers WHERE promo_id = ? ORDER BY min_qty ASC")
        .bind(&id).fetch_all(&state.db_pool).await.map_err(|e| e.to_string())?;

    Ok(PromoDetail { promo, bogo_rules, tiers })
}

#[tauri::command]
pub async fn create_promo(input: CreatePromoInput, state: State<'_, AppState>) -> Result<Promo, String> {
    ...
    for tier in input.tiers {
        ...
        .bind(&tier_id).bind(&promo_id).bind(tier.min_qty).bind(tier.discount_percent)
        .execute(&mut *tx).await.map_err(|e| e.to_string())?;
    }
    ...
    sqlx::query_as::<_, Promo>("SELECT * FROM promos WHERE id = ?")
        .bind(&promo_id).fetch_one(&state.db_pool).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn update_promo(id: String, input: CreatePromoInput, state: State<'_, AppState>) -> Result<Promo, String> {
    let mut tx = state.db_pool.begin().await.map_err(|e| e.to_string())?;
    ...
    for tier in input.tiers {
        ...
        .bind(&tier_id).bind(&id).bind(tier.min_qty).bind(tier.discount_percent)
        .execute(&mut *tx).await.map_err(|e| e.to_string())?;
    }
...
pub async fn delete_promo(id: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let in_use: Option<i64> = sqlx::query_scalar("SELECT 1 FROM sale_promo_applications WHERE promo_id = ? LIMIT 1")
...
pub async fn calculate_discounts_internal(
    ...
    let mut line_discounts_map: std::collections::HashMap<usize, Vec<AppliedDiscount>> = std::collections::HashMap::new();
    ...
    for promo in &active_promos {
        ...
        if promo.applies_to == "cart" {
            ...
            if cart_total >= promo.min_qty { // using min_qty as min_amount for cart
                let mut d = 0.0;
            ...
            for line in &lines {
                let matches_target = match promo.applies_to.as_str() {
                    "item" => promo.item_id.is_none() || promo.item_id.as_deref() == Some(&line.item_id),
                    "category" => promo.category_id.is_none() || promo.category_id.as_deref() == line.category_id.as_deref(),
                    _ => false,
                };
                ...
                if matches_target && line.qty >= promo.min_qty {
                    ...
                    match promo.promo_type.as_str() {
                        "percentage" => {
                            line_disc_val = (line.price * line.qty) * promo.discount_value.unwrap_or(promo.discount_percent) / 100.0;
                        },
                        "fixed_amount" => {
                        ...
                        "tiered" => {
                            ...
                            if let Some(tier) = tiers.iter().find(|t| t.promo_id == promo.id && line.qty >= t.min_qty) {
                                line_disc_val = (line.price * line.qty) * tier.discount_percent / 100.0;
                            }
                        },
                        "bogo" => {
                            if let Some(bogo) = bogo_rules.iter().find(|b| b.promo_id == promo.id) {
                                ...
                                if times > 0.0 {
                                    ...
                                    free_item_id = bogo.free_item_id.clone().or(Some(line.item_id.clone()));
                                    free_unit_id = bogo.free_item_unit_id.clone().or(Some(line.unit_id.clone()));
                    ...
                    if let Some(cap) = promo.max_discount_amount {
                        if line_disc_val > cap { line_disc_val = cap; }
                    }
                    ...
                    if line_disc_val > 0.0 || is_bogo {
                        ...
                        line_discounts_map.entry(line.line_index).or_insert(Vec::new()).push(applied);
                    }
                }
            }
        }
    ...
    for (line_idx, applied_list) in line_discounts_map {
        ...
        let has_best_only = applied_list.iter().any(|d| {
            active_promos.iter().find(|p| p.id == d.promo_id).map(|p| p.stack_rule.as_str() == "best_only").unwrap_or(false)
        });
        
        if has_best_only {
            // Find the single best discount
            if let Some(best) = applied_list.into_iter().max_by(|a, b| a.discount_amount.partial_cmp(&b.discount_amount).unwrap()) {
                final_list.push(best);
            }
        } else {
            // Additive or none. Just sum them up, cap at max_line_total
            for mut d in applied_list {
                let p_rule = active_promos.iter().find(|p| p.id == d.promo_id).map(|p| p.stack_rule.clone()).unwrap_or("additive".to_string());
