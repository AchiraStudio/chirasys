use crate::db::models::purchasing::{PurchaseOrder, PoLine, PoLineInput, ReceiveLineInput};
use crate::AppState;
use tauri::State;
use uuid::Uuid;

#[tauri::command]
pub async fn get_purchase_orders(branch_id: String, state: State<'_, AppState>) -> Result<Vec<PurchaseOrder>, String> {
    let query = r#"
    ...
    "#;
    sqlx::query_as::<_, PurchaseOrder>(query).bind(&branch_id).fetch_all(&state.db_pool).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn get_po_lines(po_id: String, state: State<'_, AppState>) -> Result<Vec<PoLine>, String> {
    let query = r#"
        ...
        WHERE l.po_id = ?
    "#;
    sqlx::query_as::<_, PoLine>(query).bind(&po_id).fetch_all(&state.db_pool).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn create_purchase_order(
    branch_id: String, supplier_id: String, expected_date: Option<String>, notes: Option<String>, 
    lines: Vec<PoLineInput>, state: State<'_, AppState>
...
pub async fn receive_goods(
    po_id: String, branch_id: String, supplier_id: String, invoice_no: Option<String>, 
    lines: Vec<ReceiveLineInput>, state: State<'_, AppState>
) -> Result<String, String> {
    ...
    let current_status: String = sqlx::query_scalar("SELECT status FROM purchase_orders WHERE id = ?")
        .bind(&po_id).fetch_one(&state.db_pool).await.map_err(|e| e.to_string())?;
        
    if current_status == "received" || current_status == "cancelled" {
        return Err("Cannot receive goods against a finalized or cancelled Purchase Order.".to_string());
    }

    let purchase_id = Uuid::new_v4().to_string();
    let mut total_amount = 0.0;

    for line in &lines { total_amount += line.qty_received * line.price_per_unit; }
    ...
    for line in lines {
        ...
        if line.qty_received <= 0.0 { continue; }
        ...
        let notes = format!("PO Receipt {}", invoice_no.clone().unwrap_or_else(|| "N/A".to_string()));
        sqlx::query(
        ...
        .bind(&purchase_id).bind(line.price_per_unit).bind(&line.expiry_date).bind(&line.batch_no).bind(&notes)
        .execute(&state.db_pool).await.map_err(|e| e.to_string())?;

        // Update PO Line received quantity
        sqlx::query("UPDATE po_lines SET qty_received = qty_received + ? WHERE id = ?")
            .bind(line.qty_received).bind(&line.po_line_id)
            .execute(&state.db_pool).await.map_err(|e| e.to_string())?;
    }

    // Determine new PO status (partial or received)
    let unfulfilled_lines: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM po_lines WHERE po_id = ? AND qty_received < qty_ordered")
        .bind(&po_id).fetch_one(&state.db_pool).await.map_err(|e| e.to_string())?;

    let new_status = if unfulfilled_lines > 0 { "partial" } else { "received" };

    sqlx::query("UPDATE purchase_orders SET status = ? WHERE id = ?")
        .bind(new_status).bind(&po_id).execute(&state.db_pool).await.map_err(|e| e.to_string())?;

    Ok(purchase_id)
}

#[tauri::command]
pub async fn add_purchase_payment(purchase_id: String, amount: f64, method: String, reference: Option<String>, state: State<'_, AppState>) -> Result<(), String> {
...
#[tauri::command]
pub async fn create_purchase_return(purchase_id: String, lines: Vec<ReceiveLineInput>, reason: String, state: State<'_, AppState>) -> Result<(), String> {
    let mut tx = state.db_pool.begin().await.map_err(|e| e.to_string())?;

    let return_id = Uuid::new_v4().to_string();
    let purchase = sqlx::query_as::<_, crate::db::models::purchasing::Purchase>("SELECT * FROM purchases WHERE id = ?")
    ...
    for line in lines {
        ...
        .bind(&purchase_id).bind(&line.item_id)
        ...
        if line.qty_received > (original_qty - returned_qty) {
            return Err(format!("Cannot return more than received minus already returned ({} units available)", original_qty - returned_qty));
        }
...
pub async fn get_purchases(
    ...
    let mut query_str = String::from(
        "SELECT * FROM purchases WHERE branch_id = ?"
    );
    if supplier_id.is_some() { query_str.push_str(" AND supplier_id = ?"); }
    if status.is_some() { query_str.push_str(" AND status = ?"); }
    query_str.push_str(" ORDER BY created_at DESC");

    let mut query = sqlx::query_as::<_, crate::db::models::purchasing::Purchase>(&query_str)
        .bind(&branch_id);
    if let Some(ref sid) = supplier_id { query = query.bind(sid); }
